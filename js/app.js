...
class RPSTournamentManager {
    ...
    async loadTournamentData() {
        try {
            // Load tournaments from Firebase Firestore
            ...
            for (const doc of tournamentsSnapshot.docs) {
                ...
                tournamentData.players = playersSnapshot.docs.map(playerDoc => ({
                    id: playerDoc.id,
                    ...playerDoc.data()
                }));

                // Load rounds for this tournament
                const roundsSnapshot = await firebase.firestore()
                    .collection('tournament_rounds')
                    .where('tournament_id', '==', doc.id)
                    .orderBy('round_number', 'asc')
                    .get();
                
                tournamentData.rounds = roundsSnapshot.docs.map(roundDoc => ({
                    id: roundDoc.id,
                    ...roundDoc.data()
                }));
            ...
            this.showNotification('Failed to load tournament data', 'error');
        }
    }
    ...
    updateTournamentDisplay() {
        ...
        if (this.tournaments.length === 0) {
            container.innerHTML = '<p class="text-muted">No tournaments found. Start a tournament using the RPS Tournament application.</p>';
            return;
        }

        const html = this.tournaments.map(tournament => `
            <div class="tournament-card">
                <div class="tournament-header">
                    <h4>${tournament.name || 'RPS Tournament'}</h4>
                ...
                <div class="tournament-details">
                    ...
                    <div class="detail-row">
                        <span>Mode:</span>
                        <span>${tournament.game_mode || 'Standard'}</span>
                    </div>
                    <div class="detail-row">
                        <span>Rounds:</span>
                        <span>${tournament.total_rounds || tournament.rounds?.length || 0}</span>
                    </div>
                    <div class="detail-row">
                        <span>Players:</span>
                ...
                <div class="tournament-actions">
                    <button class="btn btn-sm btn-secondary" onclick="rpsTournament.viewTournamentDetails('${tournament.id}')">
                        View Details
                    </button>
                </div>
            </div>
    ...
    updateTournamentStats() {
        ...
        const completedTournaments = this.tournaments.filter(t => t.status === 'completed').length;
        const totalRounds = this.tournaments.reduce((sum, t) => sum + (t.total_rounds || t.rounds?.length || 0), 0);
    ...
    viewTournamentDetails(tournamentId) {
        ...
        modal.innerHTML = `
            <div class="modal-content">
                <div class="modal-header">
                    <h3>${tournament.name || 'RPS Tournament'} Details</h3>
                ...
                <div class="modal-body">
                    <div class="tournament-info">
                        ...
                        <div class="info-grid">
                            ...
                            <div class="info-item">
                                <label>Started:</label>
                                <span>${new Date(tournament.start_time || tournament.created_at).toLocaleString()}</span>
                            </div>
                            ${tournament.end_time ? `
                                <div class="info-item">
                                    <label>Ended:</label>
                                    <span>${new Date(tournament.end_time).toLocaleString()}</span>
                                </div>
                            ` : ''}
                        </div>
                    </div>
                                ...
                                ${tournament.rounds.map(round => `
                                    <div class="round-item">
                                        <div class="round-header">
                                            ...
                                            <span class="round-time">${new Date(round.created_at).toLocaleString()}</span>
                                        </div>
                                        <div class="round-details">
                                            <div class="gesture-match">
                                                <span class="player-gesture">
                                                    ${round.player1_gesture} 
                                                    (${(round.player1_confidence * 100).toFixed(1)}%)
                                                </span>
                                                <span class="vs">VS</span>
                                                <span class="player-gesture">
                                                    ${round.player2_gesture} 
                                                    (${(round.player2_confidence * 100).toFixed(1)}%)
                                                </span>
                                            </div>
                                            <div class="round-result">
                                                Winner: <strong>${round.result.replace('_', ' ')}</strong>
                                            </div>
                                        </div>
                                    </div>
                                `).join('')}
    ...
    async refreshData() {
        ...
        this.showNotification('Tournament data refreshed', 'success');
    }

    // Launch RPS Tournament application
    launchTournamentApp() {
        this.showNotification('Please run the RPS Tournament application from: physical_game/main.py', 'info');
    }
}

// Initialize RPS Tournament Manager
window.rpsTournament = new RPSTournamentManager();

// Initialize app when DOM is loaded
document.addEventListener('DOMContentLoaded', () => {
    window.app = new GameHubApp();
    
    // Load tournament data when RPS Tournament page is shown
    document.querySelector('[data-page="rps-tournament"]')?.addEventListener('click', () => {
        setTimeout(() => window.rpsTournament.refreshData(), 100);
    });
});
